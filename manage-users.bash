...
while getopts 'hdau:ec' OPTION; do
    case "$OPTION" in
        d) u_del=${OPTION} ;;
        a) u_add=${OPTION} ;;
        u) t_user=${OPTARG} ;;
...
if [[ (${u_del} == "" && ${u_add} == "" && ${u_check} == "" && ${u_exists} == "") || (${u_del} != "" && ${u_add} != "") ]]; then
    echo "Please specify -a or -d and the -u and username."
fi

# Check to ensure -u is specified
if [[ (${u_del} != "" || ${u_add} != "" || ${u_check} != "" || ${u_exists} != "") && ${t_user} == "" ]]; then
...
if [[ ${u_exists} ]]; then
    result=$(grep -q "^\s*${t_user}\s" wg0.conf)
    if [[ $? -eq 0 ]]; then
        echo "The user ${t_user} exists in wg0.conf."
    else
        echo "The user ${t_user} does not exist in wg0.conf."
    fi
fi
